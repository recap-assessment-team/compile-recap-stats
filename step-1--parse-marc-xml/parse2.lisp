...
(ft "~A~%" (delim (list "barcode" "vol" "scsbid" "sharedp" "language"
                        "pubdate" "biblevel" "recordtype" "oclc" "lccn"
                        "isbn" "issn" "lccall" "localcallnum" "oh09"
...
(for-each/list /allfiles/
  ...
  (with-time
    (for-each/list (xpath /doc/ "/collection/record")
      (when (> index! 10) (die "limit reached"))
      (let ((item-info (get-item-info value!)))
        (with-get-all value!
          (scsbid sharedp language pubdate biblevel recordtype oclc lccn
           ...
           oh08 dateoflastxaction title author)
          (for-each/list item-info
            ; (ft "~A~C~A~C" (car value!) #\Tab (cadr value!) #\Tab)
            (ft "~A~C" (car value!) #\Tab)
